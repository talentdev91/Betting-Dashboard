...
module.exports = {
    async up(queryInterface, Sequelize) {
        await queryInterface.bulkInsert('leagues', [
            { id: 46, name: 'EFL Cup (England)', sportId: 1, createdAt: new Date(), updatedAt: new Date() },
        ]);
        const { readFile } = require('fs/promises');
        let teams = JSON.parse(await readFile(`src/utils/teams/england_efl_cup.json`, "utf8"));
        let teamsToInsert = [];
        ...
        for (let i = 0; i < teams.length; i++) {
            ...
            leagueTeamsToInsert.push({ teamId: teams[i].team.id, leagueId: 46, season: 2022, createdAt: new Date(), updatedAt: new Date() });
        }

...
module.exports = {
    async up(queryInterface, Sequelize) {
        await queryInterface.bulkInsert('leagues', [
            { id: 2, name: 'Champions League (UEFA)', sportId: 1, createdAt: new Date(), updatedAt: new Date() },
        ]);
        const { readFile } = require('fs/promises');
        let teams = JSON.parse(await readFile(`src/utils/teams/champions_league.json`, "utf8"));
        let teamsToInsert = [];
        ...
        for (let i = 0; i < teams.length; i++) {
            ...
            leagueTeamsToInsert.push({ teamId: teams[i].team.id, leagueId: 2, season: 2022, createdAt: new Date(), updatedAt: new Date() });
        }

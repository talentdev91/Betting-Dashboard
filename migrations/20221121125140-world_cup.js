...
module.exports = {
    async up(queryInterface, Sequelize) {
        await queryInterface.bulkInsert('leagues', [
            { id: 1111, name: 'World Cup', sportId: 1, createdAt: new Date(), updatedAt: new Date() },
        ]);
        const { readFile } = require('fs/promises');
        let teams = JSON.parse(await readFile(`src/utils/teams/world_cup.json`, "utf8"));
        let teamsToInsert = [];
        ...
        for (let i = 0; i < teams.length; i++) {
            ...
            leagueTeamsToInsert.push({ teamId: teams[i].team.id, leagueId: 1111, season: 2022, createdAt: new Date(), updatedAt: new Date() });
        }
